...
void setup() {
  pinMode(LED_BUILTIN, OUTPUT);// กำหนดให้ขาพินที่2ส่งสัณญาณ
  Serial.begin(9600);
...
void loop(){
  if(Serial.available() > 0){//ตรวจสอบว่าค่าที่รับมามากกว่า0หรือไม่
    ...
    int num = input - '0';//กำหนดตัวแปรintชื่อnumให้เท่าแปรinput เพื่อแปลงจากinputที่เป็นcharเป็นint

    if (num == 1){//ตรวจสอบถ้า numมีค่าเท่ากับ1
      digitalWrite(LED_BUILTIN, 0);//สั่งให้ขาที่2ส่งสัณญาณออกมาโดยไม่จ่ายไฟ
      Serial.println("LED ถูกปิดแล้ว");//แสดงข้อความ
    } else if (num == 2){//ตรวจสอบถ้า numมีค่าเท่ากับ2 
      digitalWrite(LED_BUILTIN, 1);//สั้งให้ขาที่2ส่งสัญญาณออกมาโดยจ่ายไฟออก
      ...
      for (int i = 0; i < 5; i++){//ขอเข้าเงื่อนไขเลข3 สั่งให้ทำลูปนี้5ครั้ง
        digitalWrite(LED_BUILTIN, 1);//สั่งให้ขาที่2ส่งสัญญาณออกมาโดยจ่ายไฟออก
        delay(300);//หน่วงเวลา
        digitalWrite(LED_BUILTIN, 0);//สั่งให้ขาที่2ส่งสัญญาณออกมาโดยไม่จ่ายไฟ
        delay(300);//หน่วงเวลา
      }
    } else {
